...
# The race-state block every writer shares (`src.objective.race_state_block`,
# moved there in V4 so the pipeline, the weekend script and the outlook write
# the same shape).  Kept under its old name: the reader and the tests know it.
race_state_block = objective.race_state_block
...
def stage_decide(args, ev, fs: dict | None = None) -> int:
    ...
    # Under V4 nothing charges this table on our own lap (kappa is 0); it is the
    # rivals' plausible stop laps and the report's "the circuit prefers lap 18".
    # The objective's own kappa is printed with the objective, below.
    ...
    rs_const = objective.measure_constants_excluding({key}) if use_rs else None
    # One objective, assembled once and splatted into every scorer below - the
    # search, the pit window, the counterfactual and the per-car plans
    # (`src.objective`).  `--no-race-state` builds the V3 baseline objective
    # instead, which is the only place kappa is non-zero.
    obj = (V4Objective.for_event(ev, cal, plan_prior=plan_prior, first_stop_tables=fs_tables,
                                 dirty_air=dirty_air, race_state=rs_const) if use_rs
           else V4Objective.v3_for_event(ev, cal, plan_prior=plan_prior, first_stop_tables=fs_tables,
                                         dirty_air=dirty_air))
    ...
    sim_kw = dict(regime=regime, step=args.pit_step, support=per_comp_support, max_per_compound=alloc["caps"],
                  max_stint=hist_caps, **obj.sim_kwargs())
    t0 = time.time()
    ...
    t0 = time.time()
    # V4: the counterfactual is judged under the objective that made the
    # recommendation - each driver's plan group's race-state term on the first
    # stop, no first-stop history prior (`objective.counterfactual_kwargs`).
    cf = strat.counterfactual(model, ev, race, float(pl.seconds), max_stint=max_stint, push=res.best["push"],
                              race_factors=cal.driver_factors, **obj.counterfactual_kwargs(res))
    timings["counterfactual_s"] = round(time.time() - t0, 1)
    if not cf.empty:
        print(f"\n  counterfactual (top 5, SC stops held fixed, {obj.version} objective, "
              f"{timings['counterfactual_s']}s):")
        _cf_cols = ["driver", "compounds", "actual_pit_laps", "model_pit_laps", "loss_s", "n_sc_stops", "classified"]
        print(cf.head(5)[_cf_cols + (["race_state_s"] if "race_state_s" in cf else [])]
