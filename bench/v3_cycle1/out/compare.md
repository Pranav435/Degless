...
| Same, V3 with the first-stop prior switched off (kappa = 0) | – | 5.000 | **4.667** | -0.333 | improved |
| Share of the field's first stops inside the model's window | 0.230 | 0.303 | **0.158** | -0.146 | regressed |
| Oracle regret of the tool's plan (s, true race rates, same cost structure) | 12.3 | 12.4 | **10.1** | -2.2 | improved |
| Oracle regret of the field's modal plan (s) | 13.8 | 15.6 | **16.7** | +1.0 | regressed |
| Oracle regret of the winner's plan (s) | 16.5 | 18.9 | **19.4** | +0.5 | not meaningfully changed |
| Weekends where the tool's plan beats the field's modal plan on the oracle | 5 | 4 | **5** | 1 | improved |
...
| Share of cars whose own plan shares the field plan's shape | – | 0.795 | **0.767** | -0.028 | not meaningfully changed |
| Per-car first-stop spread within a weekend (laps) | – | 8.857 | **7.714** | -1.143 | regressed |
...
| Live engine tick, whole field (ms, mean over the replays) | 1070.2 | 70.8 | **80.4** | +9.6 | regressed |
| Live engine tick, p95 (ms) | 1347.5 | 77.2 | **92.5** | +15.4 | regressed |
| Live engine tick, worst lap (ms) | 1408.6 | 128.6 | **153.4** | +24.8 | regressed |
| Share of real stops inside the engine's window 3 laps earlier | 0.362 | 0.442 | **0.466** | +0.024 | improved |
| Share of real stops within 3 laps of the recommendation | 0.399 | 0.533 | **0.542** | +0.009 | not meaningfully changed |
| Median |recommended in-lap − actual| (laps) | 3.750 | 2.500 | **2.500** | +0.000 | unchanged |
| 'Box now' cost the lap before the real stop (s, median) | 2.516 | 1.478 | **1.522** | +0.044 | not meaningfully changed |
| Live signal precision: window (a stop within 3 laps = true positive) | – | – | **0.176** | – | unavailable |
| Live signal recall: window | – | – | **0.463** | – | unavailable |
| Live signal precision: box_now (a stop within 3 laps = true positive) | – | – | **0.181** | – | unavailable |
| Live signal recall: box_now | – | – | **0.421** | – | unavailable |
...
| Same, with the plan prior read letter-for-letter (no C-number mapping) | – | 2 | **2** | 0 | unchanged |
| Prior-only outlook: mean share of the field on its plan | 0.329 | 0.262 | **0.276** | +0.013 | improved |
| Production posterior fit (s) | 247.372 | 6.648 | **7.559** | +0.911 | regressed |
| Weekend refit fit, 2x800x800 (s) | 21.297 | 3.580 | **4.027** | +0.447 | regressed |
| Strategy search, 500 draws, 1-lap grid, full objective (s) | 0.636 | 0.680 | **0.637** | -0.043 | improved |
| Peak resident memory of the speed benchmark (MB) | 1310.0 | 2090.0 | **1692.0** | -398.0 | improved |
| Whole benchmark suite, wall clock (s) | 1329.0 | 1329.0 | **1329.0** | +0.0 | unchanged |
| Tests passing | 20.000 | 27.000 | **37.000** | +10.000 | improved |
...
|---|---|---|---|---|---|---|---|---|---|---|
| Australia | 1-stop H-M @ 36 | 1-stop M-H @ 23 | **1-stop M-H @ 18** | 1-stop H-M @ 33 | MEDIUM-HARD (0.00→0.53→0.53) | MEDIUM-HARD | yes→yes→yes | no→yes→yes | +8 / -2 / – / -7 (SC) | 10.7 / 9.0 / 15.2 / 19.8 / 24.9 |
...
| Barcelona | 2-stop M-H-H @ 19,43 | 2-stop S-M-S @ 21,43 | **2-stop M-H-H @ 18,42** | 3-stop M-M-S-S @ 16,32,49 | MEDIUM-HARD-HARD (0.46→0.00→0.46) | SOFT-HARD-MEDIUM-HARD | yes→yes→yes | yes→no→yes | +3 / +8 / +5 / +5 | 0.1 / 10.7 / 0.2 / 5.3 / 4.7 |
| Austria | 2-stop M-H-H @ 23,47 | 2-stop M-H-M @ 22,46 | **2-stop M-H-M @ 23,48** | 3-stop S-S-M-M @ 18,36,54 | MEDIUM-HARD-HARD (0.29→0.24→0.24) | MEDIUM-HARD-HARD | yes→yes→yes | yes→yes→yes | +0 / +4 / +5 / +5 | 25.6 / 15.2 / 15.6 / 29.2 / 29.3 |
| Belgium | 1-stop S-M @ 24 | 1-stop M-S @ 20 | **1-stop M-S @ 19** | 1-stop S-M @ 23 | MEDIUM-HARD (0.00→0.05→0.05) | MEDIUM-SOFT | yes→yes→yes | no→yes→yes | +7 / +4 / – / +3 (SC) | 12.8 / 14.9 / 14.0 / 12.2 / 13.2 |
| Hungary | 2-stop M-H-M @ 23,47 | 2-stop M-H-H @ 22,46 | **2-stop M-H-M @ 22,46** | 2-stop M-M-S @ 25,51 | MEDIUM-HARD-HARD (0.00→0.32→0.00) | MEDIUM-HARD-HARD-SOFT | yes→yes→yes | yes→yes→yes | +6 / +3 / +4 / +3 | 20.8 / 32.6 / 20.8 / 35.3 / 31.7 |
...
|---|---|---|
| Tick mean (ms) | 1156 → 73 → **83** | 985 → 68 → **78** |
| Tick p95 (ms) | 1375 → 81 → **91** | 1320 → 74 → **94** |
| Stops inside the window | 0.48 → 0.54 → **0.48** | 0.25 → 0.34 → **0.45** |
| Stops within 3 laps | 0.33 → 0.50 → **0.50** | 0.47 → 0.57 → **0.58** |
| Median stop error (laps) | 4.0 → 3.0 → **3.0** | 3.5 → 2.0 → **2.0** |
| Box-now cost 1 lap before (s) | 1.8 → 0.7 → **1.0** | 3.2 → 2.3 → **2.1** |
| Signal precision / recall (V3) | window 0.19/0.59, box_now 0.17/0.52, collapse 0.14/0.04 | window 0.16/0.34, box_now 0.19/0.32, collapse 0.20/0.06 |

### Leave-one-out calibration (V3)

| Held out | Grip budget S / M / H (s) | Manage cost, floor | Grid | Dirty air | λ | τ | κ |
|---|---|---|---|---|---|---|---|
| australia-2026 | 3.84 / 4.09 / 4.19 | 0.60, 0.45 | 0.00 | 0.067 | 0.000 | 2.50 | 1.50 |
| japan-2026 | 3.84 / 4.09 / 4.30 | 0.60, 0.45 | 0.00 | 0.137 | 0.000 | 2.50 | 1.50 |
| barcelona-2026 | 3.35 / 3.27 / 4.19 | 0.60, 0.45 | 0.00 | 0.067 | 0.000 | 2.50 | 1.50 |
| austria-2026 | 3.84 / 4.09 / 4.19 | 0.60, 0.45 | 0.00 | 0.084 | 0.000 | 2.50 | 1.50 |
| belgium-2026 | 3.84 / 4.09 / 4.19 | 0.60, 0.45 | 0.00 | 0.137 | 0.000 | 2.50 | 1.50 |
| hungary-2026 | 3.84 / 4.09 / 3.81 | 0.60, 0.45 | 0.00 | 0.067 | 0.000 | 2.50 | 0.50 |
| italy-2026 | 3.84 / 4.09 / 4.19 | 0.60, 0.45 | 0.00 | 0.137 | 0.000 | 2.50 | 1.50 |
| global | 3.84 / 4.09 / 4.19 | 0.60, 0.45 | 0.00 | 0.120 | 0.000 | 2.50 | 1.50 |
...
|---|---|---|---|---|---|---|
| full | 5/7 | 7/7 | 6/7 | 0.34 | 4.3 | 1.10 |
| no_first_stop_prior | 6/7 | 7/7 | 7/7 | 0.35 | 4.7 | 1.10 |
| no_plan_prior | 2/7 | 4/7 | 4/7 | 0.08 | 1.0 | 1.10 |
| no_nomination_mapping | 4/7 | 6/7 | 5/7 | 0.28 | 3.7 | 1.10 |
| no_dirty_air_circuit | 5/7 | 7/7 | 6/7 | 0.34 | 4.3 | 1.10 |
| no_position | 5/7 | 7/7 | 6/7 | 0.34 | 4.3 | 1.10 |
| no_pace_cal | 5/7 | 7/7 | 5/7 | 0.28 | 4.0 | 1.10 |
| config_constants | 3/7 | 1/7 | 7/7 | 0.06 | 1.7 | 1.10 |
| practice_only | 3/7 | 7/7 | 5/7 | 0.15 | 3.7 | 1.04 |
| old_budget | 5/7 | 7/7 | 6/7 | 0.34 | 4.7 | 1.10 |
| no_cliff_budgets | 5/7 | 7/7 | 6/7 | 0.34 | 4.7 | 1.10 |
| no_circuit_regime_prior | 6/7 | 7/7 | 7/7 | 0.35 | 4.3 | 1.10 |
