...
from src.live.store import LIVE_DIR  # noqa: E402
...
st.set_page_config(page_title="Degless — Haas Race Strategy", layout="wide", page_icon="🏁")
inject_css()
...
def load_pq(name: str) -> pd.DataFrame:
    ...
    return _read_parquet(str(p), p.stat().st_mtime if p.exists() else 0.0)
...
st.title("Degless — Haas Race Strategy", anchor=False)
st.caption(ev.name)
render_strip(WEEKEND, DATA_BADGE)

_live = WEEKEND.get("live")
HAAS_LIVE_SK = (str(_live["key"]) if _live and (LIVE_DIR / str(_live["key"]) / "snapshot.json").exists()
               else None)
...
def _accuracy_scored() -> None:
    ...
    with more("More detail: checks and range"):
        badges([(GATE_WORDS.get(g["gate"], g["gate"].replace("MixedLM", "simple cross-check").replace("posterior", "prediction")),
             "ok" if g["pass"] else "alert") for g in meta["gates"]])
...
def _tab_accuracy() -> None:
    ...
    if not clean.empty and "session" in clean:
        fitted = ", ".join(sorted(clean["session"].dropna().unique()))
        st.markdown(f"Practice fitted: {fitted}")
    _practice_data()
    how(
